...
# 임계값. get_knee_valgus_ratio()가 반환하는 값(무릎너비/발목너비)과 비교한다.
# (2026-09-03 변경) 원래 0.8("무릎이 발목 너비의 80% 미만으로 좁아지면 눈에 띄게 모인
# 것"이라는 상식적인 잠정치, 실측 검증 없이 정한 값)이었는데, 사용자가 육안으로 확연히
# 무릎이 모인 사진을 업로드했더니 실측값이 0.890으로 나와 0.8 기준을 통과(정상 판정)해
# 버리는 걸 확인했다 — mediapipe로 직접 좌표를 뽑아 재현. 사용자 확인 후 0.9로 올림.
# 참고로 "누가 봐도 정상"인 정석 자세 참고 사진 2장의 실측값은 각각 1.064/0.974로, 0.9와도
# 여유 있게 구분된다.
# NOTE: 여전히 MVP 잠정치 — 데이터가 쌓이는 대로 사용자 신고 기반 액티브러닝으로 조정할 예정.
KNEE_VALGUS_RATIO_THRESHOLD = 0.9
