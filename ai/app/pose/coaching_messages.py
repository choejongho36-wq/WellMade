...
왜 이 문구들을 별도 파일로 분리했는가? "같은 이슈에 대해 모듈마다 표현이 미묘하게
달라지는 걸 막기 위해" 단일 출처 원칙을 쓴다 — 규칙기반 판정(pose/rules.py,
coaching/realtime.py) 전체가 이 파일의 상수를 가져다 쓴다. (RAG 지식베이스
(app/rag/knowledge_base.py)도 한때 같은 문구를 재사용했으나, RAG가 2026-09-02에 이
스코프에서 삭제되며 지금은 규칙기반 판정 모듈만 남았다.)
